...
class RawTransactionsTest(FreicoinTestFramework):
    ...
    def getrawtransaction_verbosity_tests(self):
        ...
        # Freicoin block sizes shift the -fastprune file boundaries: file 0
        # spans heights 0..~255, so prune above that (with headroom) to
        # actually remove a file.
        self.generate(self.nodes[2], 450)
        assert_greater_than(self.nodes[2].pruneblockchain(325), 0)
    ...
    def createrawtransaction_tests(self):
        ...
        # (v31 added a trailing `version` parameter, so the signature is now
        # inputs, outputs, locktime, lockheight, replaceable, version)
        assert_raises_rpc_error(-1, "createrawtransaction", self.nodes[0].createrawtransaction, [], {}, 0, -1, False, 2, 'foo')
