...
def test_no_more_inputs_fails(self, rbf_node, dest_address):
    ...
    # Freicoin: transactions carry +16 WU (4 vbytes) of extra weight, so the
    # minimum fee at this feerate is 100 kria higher than upstream's 0.00001051.
    assert_raises_rpc_error(-4, "Unable to create transaction. The total exceeds your balance when the 0.00001151 transaction fee is included.", rbf_node.bumpfee, rbfid)
    self.clear_mempool()
