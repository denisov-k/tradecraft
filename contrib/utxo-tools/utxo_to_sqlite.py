...
(txid TEXT, vout INT, value INT, coinbase INT, height INT, refheight INT, scriptpubkey TEXT)
"""
import argparse
import os
import sqlite3
...
def main():
    ...
    con.execute("CREATE TABLE utxos(txid TEXT, vout INT, value INT, coinbase INT, height INT, refheight INT, scriptpubkey TEXT)")
    ...
    block_hash = bytearray(f.read(32))
    # A set high-order bit of the base block hash indicates Freicoin's
    # extended serialization (see SnapshotMetadata in node/utxo_snapshot.h),
    # which carries the previous block-final transaction entry.
    if block_hash[31] & 0x80:
        block_hash[31] ^= 0x80
        flags = f.read(1)[0]
        if flags & 0x01:  # BlockFinalTxEntry: VARINT(size), hash if size != 0
            final_tx_size = read_varint(f)
            if final_tx_size != 0:
                f.read(32)  # final tx hash
            flags ^= 0x01
        if flags:
            print(f"Error: unknown snapshot extended serialization fields (flags={flags:#x})")
            sys.exit(1)
    ...
    for coin_idx in range(1, num_utxos+1):
        ...
        refheight = read_varint(f)
        write_batch.append((prevout_hash, prevout_index, amount, is_coinbase, height, refheight, scriptpubkey))
        ...
        if args.verbose:
            ...
            print(f"    amount = {amount}, height = {height}, refheight = {refheight}, coinbase = {is_coinbase}")
