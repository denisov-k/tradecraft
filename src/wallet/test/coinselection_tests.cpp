...
static OutputGroup MakeCoin(const CAmount& amount, bool is_eff_value = true, CoinSelectionParams cs_params = default_cs_params, int custom_spending_vsize = P2WPKH_INPUT_VSIZE)
{
    ...
    group.Insert(std::make_shared<COutput>(/*atheight=*/0, tx.vout.at(0).nValue, COutPoint(tx.GetHash(), 0), SpentOutput{tx.vout.at(0), /*refheight=*/0}, /*depth=*/1, /*input_bytes=*/custom_spending_vsize, /*solvable=*/true, /*safe=*/true, /*time=*/0, /*from_me=*/false, /*fees=*/fees), /*ancestors=*/0, /*descendants=*/0);
    return group;
}
