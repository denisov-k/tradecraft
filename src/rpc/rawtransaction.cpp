...
PartiallySignedTransaction ProcessPST(const std::string& pst_string, const std::any& context, const HidingSigningProvider& provider, std::optional<int> sighash_type, bool finalize)
{
    ...
    for (unsigned int i = 0; i < pstx.tx->vin.size(); ++i) {
        ...
        SignPSTInput(provider, pstx, /*index=*/i, &txdata, sighash_type.value_or(SIGHASH_DEFAULT), /*out_sigdata=*/nullptr, finalize);
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CMutableTransaction rawTx = ConstructTransaction(request.params[0], request.params[1], request.params[2], request.params[3], height, rbf, self.Arg<uint32_t>("version"));
