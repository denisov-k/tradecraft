...
BOOST_AUTO_TEST_CASE(merkle_test_BlockWitness)
{
    ...
    hashes.resize(vtx_count);
    // The coinbase leaf commits to the txid (the test coinbase has no inputs
    // to strip), and the final-tx leaf is its no-witness hash.
    hashes[0] = block.vtx[0]->GetHash().ToUint256();
    for (size_t pos{1}; pos + 1 < vtx_count; ++pos) {
        hashes[pos] = block.vtx[pos]->GetWitnessHash().ToUint256();
    }
    hashes[vtx_count - 1] = block.vtx[vtx_count - 1]->GetHash().ToUint256();
