...
bool CheckInputScripts(const CTransaction& tx, TxValidationState& state,
                       const CCoinsViewCache& inputs, const Consensus::Params& params, int per_input_adjustment, script_verify_flags flags, bool cacheSigStore,
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    if (VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, m_flags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.GetReferenceValue(), refheight, cacheStore, *m_signature_cache, *txdata), &error)) {
        return std::nullopt;
    } else {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase() && fScriptChecks)
        {
            ...
            if (control) {
                ...
                tx_ok = CheckInputScripts(tx, tx_state, view, params.GetConsensus(), !truncate_inputs + !use_alu, flags, fCacheResults, fCacheResults, txsdata[i], m_chainman.m_validation_cache, &vChecks);
                if (tx_ok) control->Add(std::move(vChecks));
            } else {
                tx_ok = CheckInputScripts(tx, tx_state, view, params.GetConsensus(), !truncate_inputs + !use_alu, flags, fCacheResults, fCacheResults, txsdata[i], m_chainman.m_validation_cache);
            }
            if (!tx_ok) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
